...
inline void CRC64_Init(uint64_t * crc)
{
  *crc = UINT64_C(0xffffffffffffffff);
}

inline void CRC64_Final(uint64_t * crc)
{
  *crc ^= UINT64_C(0xffffffffffffffff);
}

void CRC64_Calc(uint64_t * crc, const uint8_t * pData, size_t dlen)
{
  if (!crc || !pData || !dlen) return;
  ...
  while (dlen)
  {
    _tab_idx = (((uint8_t)(_lcrc >> 56)) ^ *pData);
    _lcrc = crc64_table[_tab_idx] ^ (_lcrc << 8);
    pData++;
    dlen--;
  }
